...
app.get('/login',
        ...
        res.render('login', {
            layout: 'login',
            csrfToken: 'disabled'    // passa o token aqui
        });
    }
);
...
app.get('/', isAuthenticated, async (req, res) => {
    try {
        ...
        // Relatório: Multas por Motorista (corrigido - usa uso_veiculos)
        const multasMotoristaResult = await query(`
          SELECT uv.motorista, COUNT(*) AS totalMultasMotorista
          FROM multas m
          JOIN uso_veiculos uv ON m.uso_id = uv.id
...
app.get('/forgot-password',  (req, res) => {
    res.render('forgot-password', { layout: 'forgot-password', csrfToken: 'disabled' });
});
...
app.get('/reset-password/:token',  (req, res) => {
    ...
    db.query("SELECT * FROM usuarios WHERE password_reset_token = ? AND password_reset_expires > ?", [token, Date.now()], (err, results) => {
        ...
        res.render('reset-password', { layout: 'reset-password', token, csrfToken: 'disabled' });
    });
});
...
app.post('/usar/:id', isAuthenticated, uploadSingleFoto,  (req, res) => {
